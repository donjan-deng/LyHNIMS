    ...
    <tbody>
        <?php foreach ($provider->models as $record) { ?>
            <tr>
                ...
                <td class="center"><?= $record['doctor_id'] == 0 ? "" : $record->doctor->name ?></td>
                ...
                <td class="center"><?= $record['arrived_at'] == 0 ? "" : '<span class="text-danger">' . date('Y-m-d', $record['arrived_at']) . '</span>' ?></td>
                ...
                <td class="center">
                    <?php if (!$record['is_arrive']) { ?>
                    <?php if(Yii::$app->user->can('record/allocate')){?>
