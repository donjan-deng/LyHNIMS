...
$this->title = '渠道管理';
$this->params['breadcrumbs'][] = '渠道管理';
?>
<div class="row">
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-edit"></i>渠道管理</h2>
            </div>
            <?php if(Yii::$app->user->can('channel/edit')){?>
            <div class="box-create"><a href="#" class="btn btn-primary" data-toggle="modal" data-target="#myDialog" data-data="add"><i class="glyphicon glyphicon-plus-sign"></i><span>添加渠道</span></a></div>
            <?php }?>
            <div class="box-content"  data-list="<?= Url::toRoute('channel/list') ?>" data-del="<?= Url::toRoute('channel/del') ?>">
...
<div class="modal fade" id="myDialog" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        ...
        $form = ActiveForm::begin([
                    'id' => 'edit-form',
                    'action' => Url::toRoute('channel/edit')
        ]);
        ?>
...
$js = <<<JS
    $(function(){
        ...
        $('#myDialog').on('show.bs.modal', function (event) {
            ...
            if(data=='add')
            {
                modal.find('.modal-title').text("添加渠道");
                $("#channel-id").val('');
                $("#channel-enabled").prop('checked',true);
            }
            else{
                modal.find('.modal-title').text("编辑渠道");
                data=eval(data);
                $("#channel-id").val(data.id);
                $("#channel-name").val(data.name);
                $("#channel-enabled").prop('checked',data.enabled==1?true:false);
            }
        });
